...
function clearHighlight() {
    ...
    this.svg.selectAll('.line.outlier-line:not(.selected)').select('path').attr(this.config.line_attributes).attr('stroke-width', this.config.marks.find(function (mark) {
        return mark.type === 'line';
    }).attributes['stroke-width'] * 4).attr('stroke', '#b2182b');
    ...
    this.svg.selectAll('.point.outlier:not(.selected)').filter(function (d) {
        return d.values.raw[0].OUTLIER == 1; //this == is intentional since 0 is prob string right now
    }).select('circle').attr(this.config.point_attributes).attr('fill-opacity', 1).attr('stroke', 'red').attr('fill', 'red').attr('r', this.config.marks.find(function (mark) {
        return mark.type === 'circle';
    }).radius * 1.5);
}
