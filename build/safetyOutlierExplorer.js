...
function highlight() {
    ...
    this.svg.selectAll('.line').sort(function (a, b) {
...
function highlight_outliers() {
    ...
    this.svg.selectAll('.line').filter(function (d) {
        return _this.outlier_id.indexOf(d.values[0].values.raw[0][_this.config.id_col]) > -1;
    }).select('path').attr('stroke-width', this.config.marks.find(function (mark) {
        return mark.type === 'line';
    }).attributes['stroke-width'] * 4).attr('stroke', '#b2182b');

    //Highlight points
    this.svg.selectAll('.point').filter(function (d) {
        return _this.outlier_id.indexOf(d.values.raw[0][_this.config.id_col]) > -1;
    }).select('circle').attr('r', this.config.marks.find(function (mark) {
        return mark.type === 'circle';
    }).radius * 1.5);
}

function maintainHighlight_outliers() {
...
function clearHighlight() {
    ...
    this.svg.selectAll('.line:not(.selected):not(.outlier)').select('path').attr(this.config.line_attributes);

    this.svg.selectAll('.outlier:not(.selected)').select('path').attr(this.config.line_attributes).attr('stroke-width', this.config.marks.find(function (mark) {
        return mark.type === 'line';
    }).attributes['stroke-width'] * 4).attr('stroke', '#b2182b');

    this.svg.selectAll('.point:not(.selected):not(.outlier)').select('circle').attr(this.config.point_attributes).attr('r', this.config.marks.find(function (mark) {
        return mark.type === 'circle';
    }).radius);

    this.svg.selectAll('.outlier:not(.selected)').select('circle').attr(this.config.point_attributes).attr('fill-opacity', 1).attr('stroke', 'red').attr('fill', 'red').attr('r', this.config.marks.find(function (mark) {
        return mark.type === 'circle';
    }).radius * 1.5);
}
...
function applyOutlier() {
    ...
    this.svg.selectAll('.line').filter(function (d) {
        return chart.outlier_id.indexOf(d.values[0].values.raw[0][_this.config.id_col]) > -1;
    }).classed('outlier', true);
    this.svg.selectAll('.point').filter(function (d) {
        return chart.outlier_id.indexOf(d.values.raw[0][chart.config.id_col]) > -1;
    }).classed('outlier', true);
}
...
function onResize() {
    ...
    }).moveToFront().select('circle').classed('outlier', true);
